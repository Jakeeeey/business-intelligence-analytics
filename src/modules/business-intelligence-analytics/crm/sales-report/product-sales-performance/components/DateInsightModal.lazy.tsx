...
export default function DateInsightModal({ period, revenue, transactions, topProducts, onClose }: {
  period: string; revenue: number; transactions?: number; topProducts: { name: string; revenue: number; transactions: number }[]; onClose: () => void;
}) {
  const fmt = fmtPHP;
  return (
      ...
      <DialogContent className="max-w-xl p-0 gap-0 overflow-hidden dark:border-zinc-700">
        <DialogHeader className="px-6 pt-6 pb-4 space-y-1">
          <div className="min-w-0">
            <DialogTitle className="text-base font-semibold leading-tight">Revenue summary for {period}</DialogTitle>
          ...
          <div className="px-6 py-4 space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg border dark:border-zinc-700 bg-muted/30 dark:bg-white/5 px-3 py-3 text-center">
                <p className="text-[11px] text-muted-foreground mb-1 leading-tight">Revenue</p>
                <p className="text-sm font-semibold tabular-nums">{fmt(revenue)}</p>
              </div>
              <div className="rounded-lg border dark:border-zinc-700 bg-muted/30 dark:bg-white/5 px-3 py-3 text-center">
                <p className="text-[11px] text-muted-foreground mb-1 leading-tight">Transactions</p>
                <p className="text-sm font-semibold tabular-nums">{(transactions || 0).toLocaleString()}</p>
              </div>
              <div />
            </div>
            {topProducts.length > 0 && (
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Top Products this period</p>
                    ...
                    <tbody>
                      {topProducts.map((p, i) => (
                        <tr key={p.name} className={i % 2 === 0 ? "" : "bg-muted/20"}>
                          ...
                          <td className="px-3 py-2 text-right">{p.transactions}</td>
                          <td className="px-3 py-2 text-right font-medium tabular-nums">{fmt(p.revenue)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
