...
import { generateCacheKey, getCacheData, setCacheData, clearExpiredCache } from "@/modules/business-intelligence-analytics/crm/sales-report/product-sales-performance/utils/cache";
...
export async function fetchProductSalesData(
  ...
  const url = `/api/bia/crm/sales-report/product-sales-performance${params.toString() ? `?${params.toString()}` : ""}`;
