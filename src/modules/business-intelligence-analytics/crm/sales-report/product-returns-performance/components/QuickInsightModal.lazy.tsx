...
function QuickInsightModal({
  ...
  const fmt = fmtPHP;
  const avg = config.item.returnCount > 0 ? config.item.returnValue / config.item.returnCount : 0;

  const relatedRows = React.useMemo(() => {
    const { type, item } = config;
    const makeMap = (filterFn: (r: ProductReturnRecord) => boolean, keyFn: (r: ProductReturnRecord) => string) => {
      ...
      filteredData.forEach((r) => {
        ...
        map.set(k, { returnValue: prev.returnValue + r.amount, returnCount: prev.returnCount + 1 });
      });
      return Array.from(map.entries()).map(([name, d]) => ({ name, ...d })).sort((a, b) => b.returnValue - a.returnValue).slice(0, 5);
    };
    // top product return modal table 
    if (type === "product") return makeMap((r) => r.productName === item.name, (r) => r.customerName);
    if (type === "supplier") return makeMap((r) => r.supplier === item.name, (r) => r.productName);
    if (type === "customer") return makeMap((r) => r.customerName === item.name, (r) => r.productName);
    if (type === "location") {
      const parts = item.name.split(", ");
      const city = parts[0];
      const province = parts.slice(1).join(", ");
      return makeMap((r) => r.city === city && r.province === province, (r) => r.productName);
    }
    return [];
  }, [config, filteredData]);

  const tableLabel =
    config.type === "product" ? "Top Customers" :
      config.type === "supplier" ? "Top Products Returned" :
        config.type === "customer" ? "Top Products Returned" :
          config.type === "location" ? "Top Products Returned" : "";
  ...
  const navTarget =
    config.type === "product" ? "product" :
      config.type === "customer" ? "product" :
        config.type === "supplier" ? "supplier" :
          "location";

  const navLabel =
    config.type === "product" ? "View in Product Tab" :
      config.type === "customer" ? "View in Product Tab" :
        config.type === "supplier" ? "View in Supplier Tab" :
      ...
      <DialogContent className="max-w-xl p-0 gap-0 overflow-hidden dark:border-zinc-700">
        <DialogHeader className="px-6 pt-6 pb-4 space-y-1">
          <div className="flex items-start gap-3 pr-6">
            <div className="mt-1 h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: config.color }} />
            <div className="min-w-0 flex-1">
              <DialogTitle className="text-base font-semibold leading-tight wrap-break-word">{config.item.name}</DialogTitle>
          ...
          <div className="px-6 py-4 space-y-4">
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: "Return Count", value: config.item.returnCount.toLocaleString() },
                { label: "Avg / Return", value: fmt(avg) },
              ].map((s) => (
                <div key={s.label} className="rounded-lg border dark:border-zinc-700 bg-muted/30 dark:bg-white/5 px-3 py-3 text-center">
                  <p className="text-[11px] text-muted-foreground mb-1 leading-tight">{s.label}</p>
            ...
            {relatedRows.length > 0 && (
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">{tableLabel}</p>
                    ...
                    <tbody>
                      {relatedRows.map((row, i) => (
                        <tr key={row.name} className={i % 2 === 0 ? "" : "bg-muted/20"}>
                          ...
                          <td className="px-3 py-2 text-right">{row.returnCount}</td>
                          <td className="px-3 py-2 text-right font-medium tabular-nums">{fmt(row.returnValue)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
